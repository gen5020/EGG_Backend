...
select nombre,precio from producto order by precio asc, nombre desc limit 1;

-- 11. Lista el nombre y el precio del producto más caro. (Utilice solamente las cláusulas ORDER BY y LIMIT)
select nombre,precio from producto order by precio desc, nombre desc limit 1;
...
orden alfabético.*/
select p.nombre,p.precio,f.nombre as fabricante from producto p, fabricante f order by f.nombre;
